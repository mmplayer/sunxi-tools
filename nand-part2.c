...
int writembrs(int fd, char names[][MAX_NAME], __u32 start, __u32 *lens, int nparts)
{
	int part_cnt = 0;
	int i;
	MBR *mbrs[MBR_COPY_NUM];
	...
	for(part_cnt = 0; part_cnt < mbr->PartCount && part_cnt < MAX_PART_COUNT; part_cnt++) {
		fprintf(backup, "'%s %d' ", mbr->array[part_cnt].name, mbr->array[part_cnt].lenlo);
	}
	fprintf(backup, "\n");
	fclose(backup);
...
int main (int argc, char **argv)
{
	...
	fd = open(nand, O_RDWR);
	if (fd < 0) {
		printf("usage: %s nand-device start1 'name1 len1' ['name2 len2'] ...\n", cmd);
		return -1;
	}

	// parse name/len arguments
	if (argc > 0) {
		if (sscanf(argv[0], "%d", &start) != 1) {
	...
	if (argc > MAX_PART_COUNT) {
		printf("too many partitions specified (MAX %d)\n", MAX_PART_COUNT);
		printf("usage: %s nand-device start1 'name1 len1' ['name2 len2'] ...\n", cmd);
		close(fd);
		return -2;
	}
	...
	if (argc > 0) {
		if (writembrs(fd, names, start, lens, argc)) {
